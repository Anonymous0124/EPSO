...
int main(int argc, char *argv[]) {
    ...
    object_out_file = in_file.substr(0, pos1 + 1) + in_filename.substr(0, pos2) + "_po_rewrite" +
                      in_filename.substr(pos2, in_filename.size() - pos2);
    report_out_file = in_file.substr(0, pos1 + 1) + in_filename.substr(0, pos2) + "_po_opt_report.txt";
    ...
    // change 'win_size_options' to adjust window size when matching rewrite rules
    vector<int> win_size_options={4};
    for(int idx=0;idx<win_size_options.size();idx++){
        auto win_size=win_size_options[idx];
        rewrite_insns_num = 0;
        rewrte_exec_time = 0;
        PeepholeOptimizer peepholeOptimizer(win_size);
        for (int i = 0; i < progs.size(); i++) {
            ...
            if(idx==0)
                 target_insns= prog.get_insns();
            else {
                target_insns=rewrite_sections[prog.get_sec_name()];
            }
            vector<Insn> rewrite_insns;
            if (i == 0) {
                bool restart=(idx==0)?true:false;
                rewrite_insns = peepholeOptimizer.optimize_with_report_output_to_path(report_out_file, sec_name,
                                                                                      prog_type,
                                                                                      attach_type, target_insns,
                                                                                      restart);
            }
            else
                rewrite_insns = peepholeOptimizer.optimize_with_report_output_to_path(report_out_file, sec_name, prog_type,
                                                                                      attach_type, target_insns,
                                                                                      false);
